 ...
 *     bod, ke kteremu se pripoji klientske zarizeni — telefon, tablet, notebook)
 ...
 *                      Tyto funkce blokovane cekaji na CAN odpovedi (az 250 ms),
 *                      proto musi bezet oddelene od WiFi stacku.
 *
 * DULEZITE UPOZORNENI:
...
/* Konfigurace presunuta do config.h */
...
static void wifi_init_ap(void)
{
    ...
    /* Captive portal DNS — presmerovani vsech DNS dotazu na ESP32.
     * Android/iOS po pripojeni k WiFi posilaji connectivity check
     * (napr. connectivitycheck.gstatic.com). DNS server presmeruje
     * vsechny domeny na 192.168.4.1, kde HTTP server odpovi spravnym
     * kodem (204/200) a zabrani oznaceni site jako "bez internetu". */
...
/**
 * @brief Callback funkce pro vsechny WebSocket udalosti (pripojeni, data, chyby).
 *
 ...
 * proto se NIKDY nesmí primo volat obd2_* funkce — ty blokovane cekaji na
 ...
 *     Zaznamená se ID klienta a jeho IP adresa. Kazdy klient dostane unikatni
 ...
 *     ztrata WiFi signalu, timeout). Zaznamená se pro diagnostiku.
 ...
 *     Přijímají se POUZE kompletni textove zpravy (ne binarne, ne fragmentovane).
 ...
 *       - info->opcode = WS_TEXT (0x01) pro textove, WS_BINARY (0x02) pro binární
 *     Fragmentovane zpravy se ignorují — vsechny JSON prikazy jsou kratke
 *     (typicky < 256 bajtu) a vejdou se do jednoho WebSocket framu.
 *     Priklad prichoziho JSON: {"cmd":"read_pid","service":1,"pid":"0x0C"}
 *
 *   WS_EVT_ERROR — chyba WebSocket protokolu (napr. neplatny frame,
 *     preteceni bufferu). Zaznamená se ID klienta.
 *
                      ...
                      AwsEventType type, void *arg, uint8_t *data, size_t len)
{
    ...
    case WS_EVT_DATA: {
         ...
         * - info->index == 0: začátek zpravy (zadny predchozi fragment)
         * - info->len == len: celkova delka odpovida prijatym datum (vse v jednom kusu)
         * - info->opcode == WS_TEXT: textovy frame (ne binarni)
         *
         * Pokud by klient poslal velkou zpravu (napr. > MTU), WebSocket ji
         * rozlozi na vice fragmentu. Takove zpravy jsou ignorovány, protoze
         * JSON prikazy jsou vzdy kratke.
         */
 ...
 *      vsechny nastavene PIDy a odesle vysledky. Pak pocka ws_get_stream_interval()
 *      milisekund (konfigurovano klientem, typicky 100-500 ms).
 *      5 ms timeout je kompromis: dostatecne kratky, aby prikazy (napr. "stop")
...
static void obd_task(void *param)
{
    ...
    while (1) {
         ...
         *   Kratky timeout umoznuje rychle reagovat na prikazy (napr. "stop_stream")
         *   i behem aktivniho streamovani. Po vyprseni timeoutu se provede
         *   jeden stream cyklus (cteni vsech nakonfigurovanych PIDu).
         *   Prikazy od klienta maji prednost pred streamem — pokud je
         *   prikaz ve fronte, zpracuje se ihned a stream se pozdrzi o jeden cyklus.
         */
        TickType_t timeout = ws_is_streaming()
                           ? pdMS_TO_TICKS(5)
                           : portMAX_DELAY;

        if (xQueueReceive(obd_request_queue, &req, timeout) == pdTRUE) {
            Serial.printf("[OBD  ] Prikaz: cmd=%d od klienta #%u\n",
                          req.cmd, req.client_id);
            ...
            if (xQueueSend(obd_response_queue, &resp,
                 ...
                 * na uvolneni mista, ale pokud ani to nestaci, zaznamená se
                 * varovani a pokracuje se. */
...
static void response_dispatch_task(void *param)
{
    ...
    while (1) {
         ...
         */
        if (xQueueReceive(obd_response_queue, &resp,
...
/**
 * @brief Hlavni inicializacni funkce Arduino frameworku — spusti se jednou pri startu.
 *
...
void setup()
{
    ...
    Serial.begin(115200);
    while (!Serial && millis() < 3000) { }  /* Cekej max 3 sekundy na pripravenost Serial portu (USB CDC) */
    Serial.println("\n========================================");
     ...
     */
    obd_request_queue  = xQueueCreate(OBD_REQUEST_QUEUE_SIZE,
                                       sizeof(obd_request_msg_t));
    obd_response_queue = xQueueCreate(OBD_RESPONSE_QUEUE_SIZE,
                                       sizeof(obd_response_msg_t));
    ...
    if (!obd_request_queue || !obd_response_queue) {
         ...
         * Program se zastavi v nekonecne smycce — nema smysl pokracovat. */
        Serial.println("[SETUP] FATAL: Nelze vytvorit fronty!");
        while (1) { delay(1000); }  /* Halt — zastaveni programu */
    }
    ...
    server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
       // request->send_P(200, "text/html", dashboard_html);
        Serial.printf("[HTTP ] GET / (Dashboard) od IP %s\n", request->client()->remoteIP().toString().c_str());
        // Explicitne zname delku pole PROGMEM, zamezi pripadnemu preteceni strlen_P a umozni prohlizeci videt Content-Length
        AsyncWebServerResponse *response = request->beginResponse_P(200, "text/html", (const uint8_t*)dashboard_html, sizeof(dashboard_html) - 1);
    ...
    /* Captive portal endpointy — odpovedi na connectivity check dotazy
     * od Android (generate_204), Apple (hotspot-detect.html) a Microsoft
     * (connecttest.txt). Bez techto odpovedi mobilni zarizeni oznaci sit
     * jako "bez internetu" a mohou automaticky odpojit WiFi. */
    ...
    /* Fallback pro vsechny ostatni connectivity check URL */
    ...
    xTaskCreatePinnedToCore(
        response_dispatch_task,
        "resp_task",
        RESPONSE_TASK_STACK_SIZE,  /* Velikost zasobniku: 6144 bajtu */
        NULL,
        3,
        &response_task_handle,
        0                          /* Jadro: Core 0 (PRO_CPU) */
    );
...
/**
 * @brief Hlavni smycka Arduino frameworku — bezi opakovaně po dokonceni setup().
 *
...
void loop()
{
    ...
    /* Obsluha captive portal DNS dotazu — neblokujici, typicky < 1 ms.
     * Musi byt volano pravidelne, jinak Android timeout na DNS
     * a oznaci sit jako nedostupnou. */
