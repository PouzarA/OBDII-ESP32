 ...
 *
 * @author Aleš Pouzar (OBD-II bakalářská práce)
 */
...
void obd2_decode_dtc_string(const uint8_t *raw, char *out)
{
    ...
    snprintf(out, 6, "%c%u%X%02X",
             type_char[type_bits], digit2, digit3, digit45);
 ...
 *   - Páry bytů: každý DTC je zakódován ve 2 bajtech (viz obd2_decode_dtc_string)
 *
 ...
 *   - max_count omezí počet rozparsovaných DTC (chrání před přetečením bufferu).
 *
 * @param sid        SID služby: OBD2_SID_READ_DTC (0x03) nebo OBD2_SID_PENDING_DTC (0x07)
 ...
 */
static obd2_status_t _obd2_read_dtc_internal(uint8_t sid,
                                              obd2_dtc_t *dtcs,
                                              uint8_t max_count,
                                              uint8_t *out_count)
{
    ...
    *out_count = 0;
    ...
    while (offset + 1 < resp_len && count < dtc_count_reported && count < max_count) {
        dtcs[count].raw[0] = resp[offset];
        dtcs[count].raw[1] = resp[offset + 1];
        obd2_decode_dtc_string(dtcs[count].raw, dtcs[count].code);
        OBD2_LOGD("read_dtc: DTC #%u = %s [%02X %02X]",
                  count + 1, dtcs[count].code,
...
obd2_status_t obd2_read_pending_dtc(obd2_dtc_t *dtcs, uint8_t max_count,
                                     uint8_t *out_count)
{
    return _obd2_read_dtc_internal(OBD2_SID_PENDING_DTC, dtcs, max_count, out_count);
}
...
/**
 * @brief Smaže všechny DTC a resetuje diagnostické informace — Mode 04 / Service $04.
 *
 ...
 *   - Pokud je závada stále přítomna, DTC se znovu objeví po 1–2 jízdních cyklech.
 *
 * Komunikační detaily:
...
/**
 * @brief Interní funkce pro čtení informací Mode 09 s NODI (Number Of Data Items).
 *
 ...
 *   - Pokud InfoType v odpovědi nesouhlasí s dotazem → OBD2_ERR_RESPONSE_MALFORMED.
 *
 * @param infotype      Kód požadované informace (např. 0x02 = VIN, 0x04 = CalID)
 * @param data_out      Výstupní buffer pro surová data (bez hlavičky SID/InfoType/NODI)
 * @param max_len       Maximální velikost výstupního bufferu v bytech
 * @param nodi_out      Výstup: hodnota NODI z odpovědi (může být NULL, pokud není potřeba)
 * @param data_len_out  Výstup: skutečná délka zkopírovaných dat v bytech
 * @return OBD2_OK při úspěchu, jinak chybový kód
 */
static obd2_status_t _obd2_read_infotype(uint8_t infotype,
                                          uint8_t *data_out,
                                          uint16_t max_len,
                                          uint8_t *nodi_out,
                                          uint16_t *data_len_out)
{
    ...
    if (nodi_out) *nodi_out = nodi;
    *data_len_out = copy_len;

    OBD2_LOGD("read_infotype: type=0x%02X NODI=%u payload=%u bytes",
              infotype, nodi, payload_len);
    return OBD2_OK;
}
...
obd2_status_t obd2_read_vin(char *vin_buf, uint8_t buf_len)
{
    ...
    obd2_status_t st = _obd2_read_infotype(OBD2_INFOTYPE_VIN,
                                            data, sizeof(data),
                                            &nodi, &data_len);
    if (st != OBD2_OK) return st;

    if (data_len < OBD2_VIN_LENGTH) {
        OBD2_LOGW("read_vin: short data (%u bytes, expected %u)",
                  data_len, OBD2_VIN_LENGTH);
        return OBD2_ERR_RESPONSE_MALFORMED;
    }
...
obd2_status_t obd2_read_ecu_name(char *name_buf, uint8_t buf_len)
{
    ...
    obd2_status_t st = _obd2_read_infotype(OBD2_INFOTYPE_ECU_NAME,
                                            data, sizeof(data),
                                            &nodi, &data_len);
    if (st != OBD2_OK) return st;

    uint8_t copy_len = (data_len > OBD2_ECU_NAME_MAX_LENGTH)
                       ? OBD2_ECU_NAME_MAX_LENGTH : (uint8_t)data_len;
 ...
 *
 * Příklady:
 *   NODI=1: jedna kalibrace, např. "CAL_ID_ENGINE_V2" (doplněno mezerami na 16 B)
 *   NODI=4: čtyři kalibrace — motor, převodovka, ABS, airbag
 *
                                ...
                                uint8_t *out_count)
{
    ...
    *out_count = 0;
    ...
    obd2_status_t st = _obd2_read_infotype(OBD2_INFOTYPE_CAL_ID,
                                            data, sizeof(data),
                                            &nodi, &data_len);
    if (st != OBD2_OK) return st;
    ...
    uint8_t count = nodi;
    if (count > OBD2_MAX_INFO_ITEMS) count = OBD2_MAX_INFO_ITEMS;
